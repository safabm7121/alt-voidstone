      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md relative z-20">
        <div ref={cardRef} className="relative border border-gray-800 bg-[#0f0f0f] p-8 overflow-hidden" onMouseEnter={() => !touchDevice && setMouseOnCard(true)} onMouseLeave={() => !touchDevice && setMouseOnCard(false)} onMouseMove={handleCardMove}>
          ...
          <div className="relative z-10">
            <div className="text-center mb-8"><h1 className="text-3xl font-black uppercase tracking-tighter text-white mb-2">Forgot Password</h1><p className="text-gray-500 font-mono text-xs">Enter your email to receive a reset code</p></div>
            {sent ? (
              <div className="text-center space-y-6">
                <div className="border border-green-500/30 text-green-400 px-4 py-3 text-sm font-mono bg-green-500/5">If an account exists with this email, a reset code has been sent.</div>
                <Link to="/reset-password" className="block w-full bg-[#ff6b35] text-black py-4 font-black text-lg uppercase tracking-[0.2em] border-2 border-[#ff6b35] hover:bg-transparent hover:text-[#ff6b35] transition-all duration-300 text-center">Go to Reset Password</Link>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button type="submit" disabled={loading} className="w-full bg-[#ff6b35] text-black py-4 font-black text-lg uppercase tracking-[0.2em] border-2 border-[#ff6b35] hover:bg-transparent hover:text-[#ff6b35] transition-all duration-300 disabled:opacity-50">{loading ? '...' : 'Send Reset Code'}</button>
              </form>
            )}
